...
#[inline]
const fn sbox(n: u8) -> u8 {
    unsafe { *SBOX.as_ptr().add(n as usize) }
}
#[inline]
const fn te0(n: u8) -> u32 {
    unsafe { *TE0.as_ptr().add(n as usize) }
}
#[inline]
const fn te1(n: u8) -> u32 {
    unsafe { *TE1.as_ptr().add(n as usize) }
}
#[inline]
const fn te2(n: u8) -> u32 {
    unsafe { *TE2.as_ptr().add(n as usize) }
}
#[inline]
const fn te3(n: u8) -> u32 {
    unsafe { *TE3.as_ptr().add(n as usize) }
}
...
pub fn key(origin: [u8; 16]) -> [u32; 44] {
    ...
    for i in 0..10 {
        x[4 * i + 4] = x[4 * i]
            ^ le_u32(
                sbox(b1(x[4 * i + 3])) ^ RC[i],
                sbox(b2(x[4 * i + 3])),
                sbox(b3(x[4 * i + 3])),
                sbox(b0(x[4 * i + 3])),
            );
        x[4 * i + 5] = x[4 * i + 1] ^ x[4 * i + 4];
        x[4 * i + 6] = x[4 * i + 2] ^ x[4 * i + 5];
        x[4 * i + 7] = x[4 * i + 3] ^ x[4 * i + 6];
    }
    x
}
