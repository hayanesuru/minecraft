...
fn try_phf_table(
    ...
    let buckets_len = table_len.div_ceil(lambda);
    ...
    buckets.sort_by_key(|(_, b)| std::cmp::Reverse(b.len()));
    ...
    // (stored in `disps`).
    let mut map = repeat_n(NOVAL, table_len).collect::<Vec<_>>();
    let mut disps = repeat_n((0, 0), buckets_len).collect::<Vec<_>>();
    ...
    let mut try_map = repeat_n(0u64, table_len).collect::<Vec<_>>();
